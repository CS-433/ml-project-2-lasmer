...
class CustomLoss(nn.Module):
    ...
    def focal_loss(self, y_true, y_pred):
        ...
        pt = torch.exp(-bce)
        focal = (self.alpha * (1-pt)**self.gamma * y_true + (1-self.alpha) * pt**self.gamma * (1-y_true)) * bce
        return focal.mean()
    ...
    def squared_dice_loss(self, y_true, y_pred, smooth=1):
        intersection = (y_true * y_pred).sum()
        return 1 - (2. * intersection + smooth) / (torch.pow(y_true, 2).sum() + torch.pow(y_pred, 2).sum() + smooth)
    ...
    def tversky_loss(self, y_true, y_pred, smooth=1):
        ...
        return 1 - (intersection + smooth) / (intersection + self.alpha * false_negatives + (1 - self.alpha) * false_positives + smooth)
    ...
    def bce_dice_loss(self, y_true, y_pred):
        return 0.3 * self.binary_cross_entropy(y_true, y_pred) + 0.7 * self.dice_loss(y_true, y_pred)

    def combo_loss(self, y_true, y_pred):
        return   self.weighted_cross_entropy(y_true,y_pred) +  self.dice_loss(y_true, y_pred) 
