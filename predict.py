...
def main(args):
    ...
    def load_unet():
        ...
        if args.use_TTA:
            return tta.SegmentationTTAWrapper(model, tta.aliases.d4_transform(), merge_mode='mean')
        else:
            return model
    ...
    def load_linknet():
        ...
        model.load_state_dict(torch.load('models/trained_model_nl_linknet_egaussian.pt'))
        if args.use_TTA:
            return tta.SegmentationTTAWrapper(model, tta.aliases.d4_transform(), merge_mode='mean')
        else:
            return model
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='predict the test images using the trained model(s).if you want to use multiple models, use --use_unet, --use_GCDCNN, --use_linknet and set them to true')
    parser.add_argument('--use_unet', type=bool, default=True, help='use true if you want to use unet (default: True)')
    parser.add_argument('--use_GCDCNN', type=bool, default=True, help='use true if you want to use GCDCNN (default: False)')
    parser.add_argument('--use_linknet', type=bool, default=True, help='use true if you want to use linknet (default: False)')
    parser.add_argument('--use_crop', type=bool, default=False, help='use true if you want to use crop (default: False)')
    parser.add_argument('--use_TTA', type=bool, default=False, help='use true if you want to use TTA (default: False)')
