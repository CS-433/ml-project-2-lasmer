...
def augment_and_save(image_path, ground_truth_path, index, aug_index, output_image_dir, output_gt_dir):
    ...
    seq = iaa.Sequential([
        ...
        iaa.Affine(
            ...
            scale={"x": (0.7, 1.3), "y": (0.7, 1.3)},  # random scaling between 0.7 and 1.3
            mode='symmetric')   # use symmetric padding for image borders after rotation
       ] )
...
def main():
    ...
    for index, filename in tqdm(enumerate(filenames), total=len(filenames)):
        ...
        for aug_index in range(NB_AUGMENTATIONS):  # Augment each image randomly multiple times( This hyperparameter can be adjusted to include more random rotations )
